...
Per story: ` + "`id`" + ` (feature = segment before the first ` + "`/`" + `, snapshot name = segment after the last ` + "`/`" + `), plus optional ` + "`feature, intent, tags, args, env, terminal, ready, readyTimeoutMs, quit, golden, steps, outcomes, variants`" + `. Generated spec name ` + "`story_<id with / -> _>`" + ` (` + "`__<variant>`" + ` suffix for a variant). Generated names must be unique across all discovered manifests and spec-file stories because run ids, goldens, and the stories index use them as storage identities. Generated steps: wait ` + "`ready`" + ` (or 300ms idle) -> the story's own ` + "`steps`" + ` -> ` + "`snapshot`" + ` -> press ` + "`quit`" + ` -> wait exit 0. Generated outcomes: ` + "`golden`" + ` (snapshot match) when golden is true, ` + "`ready`" + ` (screen match) when set, then the story's own ` + "`outcomes`" + `, else a fallback ` + "`mounted`" + ` process outcome. Tags always include ` + "`story`" + ` and ` + "`variant:<name>`" + ` for variants.
